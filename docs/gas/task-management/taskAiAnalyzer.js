...
【各タスクについて以下を整理】
1. 担当者（不明なら「未特定」）
2. 依頼者（誰が頼んだか。発言者・指示者から特定。不明なら空文字）
3. 内容（具体的に何をするか）
4. 期限（具体的な日付。不明なら「未設定」）
5. 完了条件（何がどうなっていれば完了か）
6. やること / やらないこと
7. 必要な情報・素材
8. 状態（未着手 / 進行中 / 完了済み）
9. 確信度（高 / 中 / 低）
10. 企業名（依頼元・対象の企業名。社名が文中にあれば抽出。不明なら空文字）
11. 種別（以下から最も近いものを1つ選択。不明なら空文字）: HP, LP, パンフレット・カタログ, 月刊Sing, ゆめマガ, 動画, 動画撮影, SNS関連, FB, FB資料作成, 営業支援, 採用原稿関連, 応募者対応, 名刺, ロゴ, 原稿
  ...
  "tasks": [
    {
      ...
      "task_type": "種別（HP/LP/動画/etc. 不明なら空文字）",
      "warnings": ["不明点や懸念事項"],
