...
- 分析結果をもとに列構造を大幅刷新（13列→18列、列順を一目で把握できる順に再構成）
- 企業名・種別・依頼者・ボール・親タスクID列を追加
...
- **依頼者列追加（G列）** — 誰が頼んだかを記録。AI抽出プロンプトにも追加済み
- **ボール自動切替** — ステータス変更時にボール保持者を自動更新（進行中/差し戻し→担当者、完了報告済み→依頼者or管理者）
- **一括要件定義画面** — 管理者メニューに追加。担当者別に未定義タスクを一覧、規模別レベルで処理（大:フルアシスト、中:AI叩き台）
- **タスク委譲チェーンの可視化** — タスク詳細展開時に「依頼者→担当者→ボール保持者」のチェーンを表示

#### 次のアクション（2026-03-17 改善要件）

**【バグ修正】**

1. **要件定義ダイアログが閉じてしまう** — タスク登録後（画面4）で「要件定義する」を1クリックするとダイアログが閉じる。修正して、タスク一覧（画面5）側で要件定義を行う方式にする
   - 大: 🔴必須（要件定義しないと進められない前提）
   - 中: 🟡要確認（やったほうがいいが任意）
   - 小: すぐ作業（要件定義不要）

**【要件定義フロー改善】**

2. **AI要件定義の修正→再AI呼び出し** — AIが出した要件定義に対して個別フィールドを修正し、修正内容を伝えてAI再生成できるようにする。難しければ手動フィールド編集→保存のフォールバック
   - 例: 「Sing HDのLP作成」→AIは「Sing HDのLP」と解釈→実際は「Sing HDのHP制作という商材のLP」→修正を伝えてAI再生成
3. **要件定義の貼り付け保存** — 要件定義結果を指定フォーマットでダイアログに貼り付けたら保存でき、進捗報告・確認時にチェックリストとして使えるようにする
4. **HP/LP商材の制作フローと入力項目を要件定義に反映する** — 種別がHP/LPのタスクでは、自社の制作フローを理解した前提で要件定義を行う。的外れな質問（「デザインカンプはありますか？」等）を出さない
   - **HP制作**: ヒアリングシートGAS（Part②39項目）→構成案プロンプト→テンプレートクローン（`template-*` 7種類）→Claude Code実装の流れ。要件定義時に「ヒアリング済みか」「テンプレート選定済みか」等、実際のフローに沿った確認項目を出す
   - **LP制作**: GASダイアログ入力20項目（A.基本/B.ターゲット/C.訴求/D.オファー/E.表現）→構成案プロンプト→テンプレートクローン（`template-lp`）の流れ。HPヒアリングシートからのフィールドマッピング（`LP_FIELD_MAPPING`）も活用できる前提
   - 参照: HANDOFF内「LP構成案プロンプトの設計」「HP制作 Part②ヒアリング項目」セクション

**【Claude Code連携改善】**

5. **専門用語に解説を付ける** — ディスカッション・要件定義プロンプトで専門用語（KGI等）に平易な解説を付加。覚えつつ理解もできるようにする。「KGI = 最終的に達成したいゴール（数値目標とは限らない。例: 検索上位表示、問い合わせ増加）」のような形式
6. **最終出力に確認リストを追加** — Claude Codeの最終着地として、要件定義フォーマット出力＋確認対象者ごとの確認リストを生成。確認対象者が複数いる場合はそれぞれLINE WORKS貼り付け用に分割出力する

**【UI改善】**

7. **Googleカレンダー風の直接登録ダイアログ** — AI解析を通さず、手動でタスクを直接登録できるカレンダーUIダイアログを追加
8. **タスク一覧の担当者別表示** — タスク一覧で特定の担当者のタスクだけを確認できるフィルタ/ビュー切替
9. **ピンポイント貼り付けをメニュー下部に移動** — あまり使わないため、メニュー内の優先度を下げる

**【既存の未実装】**

10. **GASにデプロイして手動テスト** — テンプレート初期設定を再実行（マイグレーションでR列追加される）→各機能の動作確認
11. **ボール自動切替のonEditトリガー対応** — シート上で直接ステータス変更した場合にもボール自動切替する（現在はダイアログ経由のみ）
...
```
主要列（左側、一目で見える）:
A:ID  B:状態  C:企業名  D:タスク名  E:種別  F:担当者  G:依頼者  H:ボール  I:期限  J:タスク規模

詳細列（右側、展開/スクロールで見る）:
K:完了条件  L:備考  M:登録日  N:入力モード  O:完了報告コメント  P:承認者/差し戻し理由  Q:要件定義  R:親タスクID
```
...
- F列（担当者）: 担当者リスト
- G列（依頼者）: 担当者リスト＋企業＋外部
- H列（ボール）: 担当者リスト＋企業＋外部（ステータス変更時に自動切替）
- J列（タスク規模）: 大, 中, 小
