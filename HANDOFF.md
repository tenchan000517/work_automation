...
## 🎯 次世代セッションタスク（2026-01-29 更新）

### HP制作マニュアル修正（優先）

**問題:** Next.jsサイトでHP制作のマニュアルページが「マニュアルが見つかりません」エラーになる

**原因:**
1. `docs/manuals/hp/` ディレクトリが存在しない
2. `src/lib/data/hp.ts` のタスク番号が連番でない（13, 18, 14, 15, 21, 22, 23, 16）
3. ガイドラインv3では「業務Noは配列順の連番」がルール

**必要な作業:**
1. `hp.ts` のタスク番号を連番（0〜7）に変更
2. `docs/manuals/hp/` ディレクトリを作成
3. 各タスクのマニュアルファイルを作成（命名規則: `00-業務名.md`）

**現在のhp.tsタスク構成:**
```
配列[0]: no="13" テンプレート選定 → no="0"に変更
配列[1]: no="18" 素材撮影 (manualDraftあり) → no="1"に変更
配列[2]: no="14" デザイン作成 → no="2"に変更
配列[3]: no="15" コーディング → no="3"に変更
配列[4]: no="21" 企業担当へ確認依頼 → no="4"に変更
配列[5]: no="22" 今後の担当者の共有とスケジュール共有 → no="5"に変更
配列[6]: no="23" HPのブログ機能の更新及びSEO最適化 (manualDraftあり) → no="6"に変更
配列[7]: no="16" 月次FB打合せ (manualDraftあり) → no="7"に変更
```

**作成済みドキュメント:**
- `docs/HP制作フローマニュアル.md` - HP制作の全体フローマニュアル（完成済み）
- `docs/manuals/common/claude-code-setup.md` - Claude Codeセットアップマニュアル（完成済み）
