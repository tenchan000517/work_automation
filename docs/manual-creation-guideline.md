...
担当者へのメンション通知を含むテンプレートは、以下の設計パターンに従う。
...
| 項目 | ルール |
|------|--------|
| **担当者ドロップダウン** | 設定シートA列のメンバー一覧から生成 |
| **担当者デフォルト値** | 設定シートの業務担当者（例: No.7撮影→川崎） |
| **CCドロップダウン** | 同じくメンバー一覧から生成 |
| **CCデフォルト値** | B列備考に「CC」を含むメンバー（例: 青柳） |
| **テンプレート** | 担当者/CC選択で動的生成 |
| **プレーンテンプレート** | プレースホルダー付き（`{{撮影担当}}`等）をアコーディオンで表示 |

#### 7.5.3 GAS実装パターン

```javascript
// 設定取得
const members = getMemberList();           // A列のメンバー一覧
const settings = getSettingsFromSheet();   // 業務担当者マッピング
const defaultTantou = settings['撮影担当'] || '';  // No.7の担当者
const defaultCC = settings['CC'] || '';    // B列に「CC」があるメンバー

// HTML側
<select id="tantouSelect">
  <!-- membersからオプション生成、defaultTantouをselected -->
</select>
<select id="ccSelect">
  <!-- membersからオプション生成、defaultCCをselected -->
</select>
...
#### 7.5.4 プレーンテンプレート（アコーディオン）

プレースホルダー付きのテンプレートを、展開可能なアコーディオンで表示し、コピーできるようにする。
