...
### LP構成案プロンプト生成: GASダイアログ修正（バグあり）

#### 現在の状態

- LP構成案プロンプト生成機能を `docs/gas/hp/lpCompositionPrompt.js` に新規実装済み
- メニュー追加済み（`hearingSheetManager.js` の `4️⃣ 構成案作成` 内、区切り線の下）
- **GASにデプロイして動作確認したところ、複数の問題あり**

#### 既知のバグ・要修正

1. **企業ドロップダウンが表示されない**
   - `lp_showCompositionDialog()` → `hp_getCompanySheetListForJsonOutput()` でシートリスト取得 → `lp_createCompositionDialogHTML(sheetData)` に渡してる
   - HTML内の `<script>` で `var sheetData = ${sheetDataJson};` → `window.onload` でドロップダウン生成
   - 原因を調査して修正が必要（JSON渡しの問題 or onload時の処理順序 or sheetDataの構造不一致）
2. **その他、実際のGAS上で動作確認して出る問題を潰す必要あり**
   - テンプレートリテラルのエスケープ問題は一部修正済み（`String.fromCharCode(96,96,96)` で回避）
   - 全20フィールドの入力→生成→コピーの一連フローをGAS上で通しテスト

#### 実装済み機能

| 機能 | 状態 | 備考 |
|------|------|------|
| 20項目入力ダイアログ | ✅ 実装済み | ドロップダウン6項目 + 自由記述14項目 |
| LP構成案プロンプト生成 | ✅ 実装済み | 2専門家モデル、9セクション構成 |
| Claude Code指示文生成 | ✅ 実装済み | template-lp クローン方式 |
| 企業シート選択→自動反映 | ⚠️ バグあり | Part①②からLP項目にマッピング |
| Part④にLP入力データ保存 | ✅ 実装済み | ラベル「LP制作データ」でJSON保存 |
| 前回データ復元 | ✅ 実装済み | シート選択時に自動復元 |
| プロンプトにPart①②全JSON付加 | ✅ 実装済み | 「参考情報」セクションとして末尾に |
| メニュー追加 | ✅ 実装済み | 4️⃣構成案作成 内 |

#### ファイル構成

| ファイル | 変更内容 |
|---------|---------|
| `docs/gas/hp/lpCompositionPrompt.js` | ★新規。LP構成案プロンプト生成の全機能 |
| `docs/gas/hp/hearingSheetManager.js` | メニューにLP項目追加（1行） |
| `docs/samples/lp/tsunageru-service-overview.md` | ★新規。ツナゲルLP用テストデータ + サービス概要 |
| `tsconfig.json` | `exclude` に `"docs"` 追加（GAS JSファイルのTS誤検出防止） |

#### LP構成案プロンプトの設計

**入力20項目（ドロップダウン / 自由記述）:**
- A.基本: 企業名、サービス名、CV種別(DD)、流入元(DD)
- B.ターゲット: ターゲット像、課題、検索意図、比較検討ポイント
- C.訴求: 提供内容、強み、実績、競合、お客様の声
- D.オファー: 価格(DD+詳細)、特典(DD+詳細)、保証(DD+詳細)
- E.表現: トーン(DD)、ブランドカラー、参考LP、NG表現

**9セクション構成（テンプレート側と合意済み）:**
1. ファーストビュー — キャッチコピー + CTA
2. 課題提起 — ターゲットの悩みに共感
3. 解決策提示 — サービス/商品で解決できる理由
4. 特徴・強み — 3カラムで訴求
5. 実績・数字 — 社会的証明
6. お客様の声 — テスティモニアル
7. 料金/オファー — 特典・限定性
...
**HP→LPフィールドマッピング（`LP_FIELD_MAPPING` 定数）:**
- メインターゲット → ターゲット像
- 抱えている課題・悩み → 課題・悩み
- 自社の強み + 選ばれる理由 + サービス強み + 独自技術 → 強み（結合）
- 検索しそうなキーワード → 検索意図（追記）
- 等（詳細はコード内の `LP_FIELD_MAPPING` 参照）

**テンプレートリポジトリ:** `tenchan000517/template-lp`（GitHub、構築中）

#### テスト用データ

`docs/samples/lp/tsunageru-service-overview.md` にツナゲルのテストケース20項目あり。

---

### タスク管理システム GAS: 大規模改修中（2026-03-17〜）

#### 現在の状態

- 既存タスク管理スプレッドシート（「制作陣タスク」15シート）の完全分析完了
- 分析結果をもとに列構造を大幅刷新（13列→17列、列順を一目で把握できる順に再構成）
- 企業名・種別・ボール・親タスクID列を追加
- 完了ログシート追加（完了タスクの自動移行）
- 進捗報告機能追加（要件定義のscope_in/outに対するチェック→LINE WORKS報告生成）
- Claude Code連携: `/define-task`コマンド依存を廃止 → プロンプト埋め込み方式に変更
- テンプレート初期設定にマイグレーション機能追加（再実行しても既存データを保護）

#### 次のアクション（未実装）

1. **依頼者列の追加** — 「誰が頼んだか」を記録。タスク委譲の可視化に必須
2. **ボール自動切替** — ステータス変更時にボール保持者を自動更新
3. **一括要件定義画面** — 担当者別に未定義タスクを一覧、規模別レベルで処理
   - 大: フルアシスト（AI叩き台→対話深掘り→確定必須）
   - 中: 中程度（AI叩き台→必要なら深掘り→確定推奨）
   - 小: 最小（完了条件だけ自動生成→即確定）
4. **タスク委譲チェーンの可視化** — 役員→中間管理職→作業者へのパスが一目でわかる表示

#### 既存ツール分析結果

分析レポート: `docs/plans/task-management/existing-tool-raw-report.md`
分析対象: https://docs.google.com/spreadsheets/d/1n_GJeOQOA_XrOLj3wO_qPbwrumrS99OC-dbzi4ACJ5g/
抽出GAS: `docs/gas/task-management/spreadsheetAnalyzer.js`

既存ツールの主な問題（分析で判明）:
- 同じタスクが5シート以上に分散（河合さん系3シート + 月別 + 週間スケジュール）
- ステータス表現がシートごとにバラバラ（完了/納品済/タスク完了/処理中...）
- 背景色による管理（暗黙知、どこにも定義なし）
- 非表示行で完了管理（アーカイブの仕組みなし）
- 管理者が全体把握するのに5シート横断が必要

既存ツールから活かした設計:
- 企業名 × 種別（中項目DD）= 現場が既に使い慣れた分類軸
- ボール列 = 既存2月/3月タスクの最優秀機能（誰で止まってるかの可視化）
- 目標工数 vs 実工数 = 河合さんが既に運用していた概念
...
**要件定義機能（✅ 実装完了）:**
- タスク規模（大/中/小）をAIが自動判定 → I列に保存
- ハードブロックはしない設計（ブロックすると「小」に変更して回避される）
- 登録完了画面（画面4）・タスク一覧（画面5）の両方に「📝 要件定義する」ボタン
- 要件定義ダイアログ: AIが叩き台生成（KGI・手離れ定義・スコープ・確認事項） → 編集 → P列にJSON保存
- Claude Code連携: 「🤖 Claude Codeで深掘り」ボタン → **プロンプト埋め込み方式**（コピペするだけで要件定義が始まる）
- 気づかせる仕組み: 大🔴・中🟡バッジ、サマリーバーに「📝要件定義待ちX件」、リマインドメール・週次棚卸しにも追加
- Claude Codeコピー: 要件定義未完了→プロンプト付き全文コピー、完了→KGI・手離れ定義を含む完全版

**進捗報告機能（✅ 実装完了）:**
- GASメニュー「📊 進捗報告」から任意タイミングで起動
- 自分の進行中タスクを選択 → 要件定義のscope_inがチェックリストで表示
- scope_out（やらないこと）を警告表示 → スコープ逸脱防止
- KGIを常に表示（ゴールを見失わない）
- LINE WORKS用の報告フォーマットを自動生成 → コピーして貼るだけ

**完了ログ機能（✅ 実装完了）:**
- 完了承認時にタスクデータを「完了ログ」シートに自動移行＋タスク一覧から行削除
- タスク一覧には生きてるタスクだけ残る（行が溜まらない）
- 完了ログはタスク一覧と同構造＋完了日時列 → AIサマリーの実績DBとして使用可能

---

**実装完了:** 13ファイル（`docs/gas/task-management/`）
...
| シート | 役割 |
|--------|------|
| タスク一覧 | 生きてるタスクのみ（完了タスクは完了ログへ移行） |
| 設定 | 担当者・システム設定 |
| ログ | AI入力/出力の記録 |
| **完了ログ** | **完了タスクの実績DB（タスク一覧と同構造＋完了日時）★新規** |

#### 列構造（17列、A〜Q）

```
主要列（左側、一目で見える）:
A:ID  B:状態  C:企業名  D:タスク名  E:種別  F:担当者  G:ボール  H:期限  I:タスク規模

詳細列（右側、展開/スクロールで見る）:
J:完了条件  K:備考  L:登録日  M:入力モード  N:完了報告コメント  O:承認者/差し戻し理由  P:要件定義  Q:親タスクID
```

**ドロップダウン:**
- B列（状態）: 未着手, 進行中, 完了報告済み, 完了, 差し戻し, 保留
- E列（種別）: HP, LP, パンフレット・カタログ, 月刊Sing, ゆめマガ, 動画, 動画撮影, SNS関連, FB, FB資料作成, 営業支援, 採用原稿関連, 応募者対応, 名刺, ロゴ, 原稿
- G列（ボール）: 担当者リスト＋企業＋外部
- I列（タスク規模）: 大, 中, 小
...
| `taskSheetManager.js` | シート操作・CRUD・テンプレート初期設定・マイグレーション・完了ログ移行 |
| `taskAiAnalyzer.js` | Gemini API呼び出し・プロンプト・JSON解析（企業名・種別抽出対応） |
| `taskCalendarManager.js` | カレンダーイベント作成・更新・削除 |
| `taskInputDialog.js` | 画面1：モード選択+テキスト入力 |
| `taskResultDialog.js` | 画面2〜4：候補選択→要件確認→登録完了（企業名・種別フォーム追加） |
| `taskListDialog.js` | 画面5：タスク一覧 + 要件定義ダイアログ（プロンプト埋め込み方式） |
| `taskProgressReport.js` | **進捗報告（scope_in/outチェック→LINE WORKS報告生成）★新規** |
| `taskReminder.js` | リマインド・通知・トリガー管理 |
| `taskSimulator.js` | ユニットテスト（15カテゴリ・130+テスト） |
| `taskFlowTestScenarios.md` | フローテストシナリオ集（20シナリオ） |
| `spreadsheetAnalyzer.js` | **既存ツール分析GAS★新規** |
| `test-data/` | **テスト用入力テキスト（5パターン）★新規** |
...
| 機能 | シミュ | 手動 | 備考 |
|------|:------:|:----:|------|
| テンプレート初期設定 | ✅ | ✅ | 3シート+条件付き書式+ドロップダウン+L/M列追加 |
| ワークス一括登録 | ✅ | ✅ | AI解析→候補選択→要件確認→登録完了 |
| Claude Codeコピー | ✅ | ✅ | 登録完了画面+タスク一覧+要件定義情報含む |
| タスク一覧（全員用） | ✅ | ✅ | 担当者別グルーピング+フィルタ+規模バッジ |
| 完了承認（管理者用） | ✅ | ✅ | ビュー切替実装済み |
| ログ記録 | ✅ | ✅ | 行高さ固定（21px） |
| タスク規模判定 | ✅ | ⬜ | AI自動判定（大/中/小）→L列保存 |
| 要件定義ダイアログ | ✅ | ⬜ | AI叩き台→編集→M列JSON保存 |
| 要件定義バッジ・サマリー | ✅ | ⬜ | 🔴大/🟡中/✅完了バッジ、サマリーバー |
| 担当者設定 | ✅ | ⬜ | |
| リマインド設定 | ✅ | ⬜ | |
| 棚卸し設定 | ✅ | ⬜ | |
| AIモデル設定 | ✅ | ⬜ | |
...
GASプロジェクトに追加して「🧪 シミュレーション」メニューから実行。15カテゴリ・130+テスト。
...
| 時刻設定が読み取り時にDate文字列になる | Sheetsが「10:00」をDate型に自動変換 | task_getSystemSettingでinstanceof Date判定→HH:mm形式に変換 |
