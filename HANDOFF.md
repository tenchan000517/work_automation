...
1. **フローテスト継続**（シナリオ2〜20。シナリオ1は✅完了・修正済み）
2. **要件定義機能の設計・実装**（フローテストで見えた最重要課題）
   - AIによるタスク規模（大/中/小）自動判定
   - 登録完了画面・タスク一覧に「📝 要件定義する」ボタン追加
   - Claude Code連携（skill、タスク管理ディレクトリ）
   - 気づかせる仕組み（一覧ラベル、リマインド、AIサマリーに1行追加）
   - 詳細は下記「要件定義とは」「要件定義機能の設計方針」を参照
