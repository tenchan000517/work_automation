...
## 次回セッションへの引き継ぎ（2026-01-14）

### フローステップの改善

1. **タイトルを簡潔にし、summaryで補足する**
   - 現状: タイトルが長くてボックスが大きくなりがち
   - 改善: タイトルは短く、詳細はsummaryフィールドに入れる
   - 例: 「LINEWORKSで企業グループを作成」→ タイトル「グループ作成」+ summary「LINEWORKSで企業グループを立ち上げ」

2. **タイトルを9商材共通化**
   - 共通で使えるタイトル例: 「企業情報入力」「日程調整」「打合せ準備」「議事録作成」「構成案作成」「撮影」「編集」「原稿作成」「確認依頼」「FB対応」等
   - 商材固有の部分はsummaryで補足

3. **ボタンの絵文字を削除**
   - 現状: `📄 {link.label}` や `📋 {link.label}` で絵文字が入っている
   - 問題: ボックスが狭いと絵文字で改行されて見づらい
   - 修正箇所: `TaskCard.tsx` の `flow-step-link-btn` と `flow-step-popup-btn` のテキスト部分

**参考ファイル:**
- `src/components/TaskCard.tsx` - フローステップのボタン部分（340-370行目付近）
- `src/lib/data/tsunageru.ts` - フローステップのデータ（タイトル変更対象）
